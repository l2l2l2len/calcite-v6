...
interface FootingCalcProps {
  onBack: () => void;
  onAddBOQ: (item: Omit<BOQItem, 'id' | 'projectId' | 'timestamp' | 'currencySymbol'>) => void;
}

const FootingCalc: React.FC<FootingCalcProps> = ({ onBack, onAddBOQ }) => {
  ...
  const results = useMemo(() => {
    ...
    const W = width;
    const D = depth / 1000;
    const volume = L * W * D * qty;
    const dryVol = volume * 1.54;
    const mix = MIX_RATIOS['M20'];
    ...
    const cost = volume * 7000;

    return { volume, cementKg, sandM3, aggM3, cost };
  }, [length, width, depth, qty]);
    ...
    <div className="px-5 py-6">
      <div className="flex items-center gap-4 mb-7">
        ...
        <div>
          ...
          <h1 className="font-serif text-2xl font-bold text-gray-900 dark:text-white">Footing Calculator</h1>
        </div>
      </div>
      ...
      <div className="space-y-5 mb-8">
        {[
          ...
          { label: 'Number of Footings', val: qty, set: setQty },
        ].map((f, i) => (
      ...
      <div className="bg-white dark:bg-gray-800 rounded-[24px] overflow-hidden shadow-xl border border-coral/10 dark:border-white/5">
        ...
        <div className="p-5 grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 p-4 bg-mint/5 rounded-2xl border-l-4 border-mint">
            <div className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Est. Cost (M20)</div>
            <div className="font-mono text-2xl font-bold text-gray-900 dark:text-white">₹{Math.round(results.cost).toLocaleString()}</div>
          </div>
        </div>
        <div className="p-5">
          <button
            onClick={() => onAddBOQ({ name: 'RCC Footing', detail: `${qty}nos x ${length}x${width}m`, amount: results.cost, type: 'footing' })}
